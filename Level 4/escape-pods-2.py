def solution(entrances, exits, path):
    ...
    ans = 0
    middleRooms = path[ne:n-nx]                
    l = n-nx-ne # number of middle rooms

    # print(middleRooms)
    
    for i in range(l):                 
        
        # max bunnies which can pass through this room
        maxPossible = 0
        for j in range(n-nx,n):
            maxPossible += middleRooms[i][j]  
        # print(maxPossible)    
        incoming = 0                             
        
        for j in entrances:
            # number of bunnies which enter the middle rooms
            incoming += path[j][ne+i] 

        ans += min(incoming, maxPossible)
    return ans
...
#  You need to get the numerous bunnies through the various rooms to the escape pods. Unfortunately, the corridors between the 
#  rooms can only fit so many bunnies at a time. What's more, many of the corridors were resized to accommodate the LAMBCHOP, 
#  so they vary in how many bunnies can move through them at a time. 

# Given the starting room numbers of the groups of bunnies, the room numbers of the escape pods, and how many bunnies can fit 
# through at a time in each direction of every corridor in between, figure out how many bunnies can safely make it to the 
...
# the corridors, returning the total number of bunnies that can get through at each time step as an int. 
